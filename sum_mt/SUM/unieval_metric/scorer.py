...
class UniEvaluator:
    def __init__(self, model_name_or_path, max_length=1024, device=None, cache_dir=None):
        """ Set up model """
        self.device = 'cpu'
        ...
        for i in tqdm(range(0, len(inputs), batch_size)):
            ...
            try:
                with torch.no_grad():
                    ...
                    src_tokens = encoded_src['input_ids'].to('cpu')
                    src_mask = encoded_src['attention_mask'].to('cpu')

                    tgt_tokens = encoded_tgt['input_ids'].to('cpu')[:, 0].unsqueeze(-1)
