...
class Scorer:
    ...
    def __init__(self, args, file_path, device='cpu'):
        ...
        if args.save_spacy_trf:
            ...
            transform_d['spacy_trf_d'] = torch.load(spacytrf_save_fn, weights_only=False)
    ...
    def score(self, metrics, sys_lines):
        ...
        for metric_name in metrics:
            if metric_name == 'bleu':
                ...
                def bleurt_score_batch(references, candidates):
                    scores = []
                    for ref, cand in zip(references, candidates):
                        inputs = tokenizer(ref, cand, return_tensors="pt", truncation=True, padding=True)
                        with torch.no_grad():
                ...
                scores_better = bleurt_score_batch(self.refs, sys_lines)
                res_d[metric_name] = np.mean(scores_better)
                ...
                start = time.time()
                scores_better_seg, scores_better = model.predict(hyp1_samples, num_workers = 0)
...
def main():
    ...
    res_d = {}
    print("scorer.sys_names: ", scorer.sys_names)
